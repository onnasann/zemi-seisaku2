function BackCircle({
    ...
    previewCostRange = null // [min, max] — スライダー操作中の未確定範囲
}) {
    ...
    const getAngle = (cost) => {
        if (costRange) {
            ...
            if (span <= 0) return 90;
            return (1 - (cost - costRange.min) / span) * 180;
        }
        if (cost > 0) {
    ...
    // 未確定のガイドは、全データの尺度上で動かす。
    // これによりスライダー操作中も点線の角度が連続して変化する。
    const getPreviewAngle = (cost) => {
        if (cost > 0) {
            const ratio = maxCost > 0 ? cost / maxCost : 0;
            return 90 - Math.min(1, Math.max(0, ratio)) * 90;
        }
        if (cost < 0) {
            const ratio = minCost < 0 ? Math.abs(cost) / Math.abs(minCost) : 0;
            return 90 + Math.min(1, Math.max(0, ratio)) * 90;
    ...
    const visibleCostBoundaries = costRange
        ? [0.25, 0.5, 0.75].map((ratio) => {
            const value = costRange.min + (costRange.max - costRange.min) * ratio;
            return {
                value,
                label: `輩出力 ${value >= 0 ? "+" : ""}${value.toFixed(1)}`
            };
        })
        : costBoundaries;
    ...
    return (
        <g className="back-circle-group">
            ...
            {/* ==================================
                輩出力 境界線 & ラベル
            ================================== */}
            {visibleCostBoundaries.map((boundary) => {
            ...
            {/* ==================================
                範囲選択モード: 両端ラベル
                左端(180°) = 最小輩出力, 右端(0°) = 最大輩出力
            ================================== */}
            {(previewCostRange || costRange) && (() => {
                const guideRange = previewCostRange
                    ? { min: previewCostRange[0], max: previewCostRange[1] }
                    : costRange;
                const minRad = previewCostRange
                    ? (getPreviewAngle(guideRange.min) * Math.PI) / 180
                    : Math.PI;
                const maxRad = previewCostRange
                    ? (getPreviewAngle(guideRange.max) * Math.PI) / 180
                    : 0;
                const labelRadius = maxRadius + 16;
                    ...
                    <g>
                        ...
                        <g transform={`translate(${minLabelX}, ${minLabelY})`}>
                            ...
                            <text x="0" y="4" textAnchor="middle" fontSize="9.5" fontWeight="800"
                                ...
                                {guideRange.min >= 0 ? "+" : ""}{guideRange.min.toFixed(1)}（低）
                            </text>
                        </g>
                        ...
                        <g transform={`translate(${maxLabelX}, ${maxLabelY})`}>
                            ...
                            <text x="0" y="4" textAnchor="middle" fontSize="9.5" fontWeight="800"
                                ...
                                {guideRange.max >= 0 ? "+" : ""}{guideRange.max.toFixed(1)}（高）
                            </text>
                        </g>
                    </g>
                );
